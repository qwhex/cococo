...
def _heaviest_span(
    ...
    Returns ``(start_line, end_line, points)`` or None. Requiring ``< total`` avoids
    "extract the whole function"; requiring at least half the points keeps the pointer
    on a substantial chunk, not a stray statement.
    """
    ...
    best = _best_run(points, floor, total)
    if best is None:
        return None
    run_points, i, j = best
...
def _best_run(points: list[int], floor: int, total: int) -> tuple[int, int, int] | None:
    """Max-points contiguous run in ``[floor, total)``; earliest wins ties."""
    best: tuple[int, int, int] | None = None
    for i in range(len(points)):
        cand = _best_run_from(points, i, floor, total)
...
def _best_run_from(
    points: list[int], start: int, floor: int, total: int
) -> tuple[int, int, int] | None:
